...
export class AIController {
  static async scoreCredit(req: AuthRequest, res: Response): Promise<void> {
    try {
      const tenantId = req.tenant!.id;
      const { loanId, features } = req.body as ScoreCreditRequest;
      ...
      const loanResult = await pool.query(
        'SELECT * FROM loan_cases WHERE loan_id = $1 AND tenant_id = $2',
        [loanId, tenantId]
      );
      ...
      const result = await AIService.scoreCredit(tenantId, loanId, features || {});
      res.json(result);
    } catch (error) {
  ...
  static async parseDocument(req: AuthRequest, res: Response): Promise<void> {
    try {
      const tenantId = req.tenant!.id;
      const { documentId, extractFields } = req.body as ParseDocumentRequest;
      ...
      const result = await AIService.parseDocument(tenantId, documentId, extractFields || []);
      res.json(result);
    } catch (error) {
  ...
  static async getModelRuns(req: AuthRequest, res: Response): Promise<void> {
    try {
      const tenantId = req.tenant!.id;
      ...
      logger.error('Get model runs error:', error);
  ...
  static async getDashboardMetrics(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const modelMetricsResult = await pool.query(`
        SELECT 
          model_id,
          COUNT(*) as total_runs,
      ...
      const scoringDistributionResult = await pool.query(`
        SELECT 
          CASE 
      ...
      const automationMetricsResult = await pool.query(`
        SELECT 
          action_type,
          COUNT(*) as count,
