...
import { WorkflowDefinition, Case, UserRole, CaseStatus } from '../types';
...
export class WorkflowService {
  ...
  static async createWorkflowDefinition(
    ...
    try {
      ...
      const versionResult = await client.query(
        'SELECT COALESCE(MAX(version), 0) + 1 as next_version FROM workflow_definitions WHERE tenant_id = $1 AND workflow_id = $2',
        [tenantId, definition.workflowId]
      );
      ...
      if (definition.isActive) {
        await client.query(
          'UPDATE workflow_definitions SET is_active = false WHERE tenant_id = $1 AND workflow_id = $2',
          [tenantId, definition.workflowId]
        );
      }
  ...
  static async executeTransition(
    ...
    try {
      ...
      await AuditService.log(
        ...
        'case',
        caseId,
        `TRANSITION_${action.toUpperCase()}`,
        { stage: currentCase.current_stage, data: currentCase.data },
        { stage: newStage, data: newData },
        { transition: transition.id, action }
      );
  ...
  private static async executeAutoRules(
    tenantId: string,
    caseId: string,
    stage: string,
    workflow: WorkflowDefinition
  ): Promise<void> {
    const autoRules = workflow.autoRules?.filter(rule => 
      rule.stage === stage && rule.trigger === 'onEnter'
    );
    ...
    for (const rule of autoRules) {
      try {
        if (rule.action.startsWith('call:ai/')) {
          ...
          await AIService.callAIService(tenantId, caseId, aiAction, rule.params);
        }
        
        logger.info(`Executed auto rule ${rule.id} for case ${caseId}`);
      } catch (error) {
        logger.error(`Failed to execute auto rule ${rule.id}:`, error);
        // Continue with other rules even if one fails
      }
    }
  }

  static async getAllWorkflows(tenantId: string): Promise<WorkflowDefinition[]> {
