...
const migrations = [
    {
      ...
      -- Create RLS policies for tenants (allow service role insert)
      CREATE POLICY "allow service role insert" ON tenants FOR INSERT TO public WITH CHECK (true);
      CREATE POLICY "service role insert" ON tenants FOR INSERT TO public WITH CHECK (role() = 'service_role');
    `
    }
];
const runMigrations = async () => {
    try {
        ...
        const { error: createTableError } = await connection_1.supabase.rpc('exec_sql', {
            sql: `
      ...
      `
        ...
        const { data: executedMigrations, error: selectError } = await connection_1.supabase
            .from('migrations')
            .select('version')
            .order('version');
        if (selectError) {
        ...
        for (const migration of migrations) {
            if (!executedVersions.has(migration.version)) {
                ...
                const { error: migrationError } = await connection_1.supabase.rpc('exec_sql', {
                    sql: migration.sql
                });
                if (migrationError) {
                ...
                const { error: insertError } = await connection_1.supabase
                    .from('migrations')
                    .insert({
                ...
                logger_1.logger.info(`Migration ${migration.version} completed successfully`);
            }
        }
